...
安全定义：由主机发起的命令安全与否由谁决定。
安全过滤：在其内部阻断违规事务传递的组件。

## CPU的安全定义与过滤

主机侧的安全过滤组件是指连接在主机与总线之间，会起到阻断主机数据传播到总线上的组件。
因为cpu之间连接到总线上，所以严格来说，没有过滤组件。sau和mpu是在cpu内部起过滤作用的，暂且放在这个位置。

sau的安全过滤作用在于：当非安全态试图跨权访问安全区域时，SAU在指令发送至总线前将其强制阻断，同时触发硬件安全异常，确保敏感数据不被泄露。
而idau没有阻断的功能。
